...
def save_data(R, G, B, fn_head, composite):
    tif_fn_data = data_dir + 'data/{}_{}.tif'.format(composite, fn_head)
...
def get_scn(fns, extent, composites):
...
def save_composite(composite, scn, fn_head):
    ...
    G = corr_data[0][1]
    B = corr_data[0][2]
    data_saved = save_data(R, G, B, fn_head, composite)
